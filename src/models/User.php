...
class User extends Model
{
    ...
    function dataTbleCountTotalUsersRow(array $whereParams, $likeParams = [])
    {
        ...
        $sql = "SELECT us.*, fn.* 
            FROM " . TABLES::USERS . " us JOIN " . TABLES::FONCTIONS . " fn  ON fn.code_fonction = us.fonction_code $where";
        $stmt = $this->db->prepare($sql);
    ...
    function DataTableFetchUsersListe($likeParams = [], int $start = 0, int $limit = 10)
    {
        ...
        $sql = "SELECT us.*, fn.* 
            FROM " . TABLES::USERS . " us JOIN " . TABLES::FONCTIONS . " fn  ON fn.code_fonction = us.fonction_code $where ORDER BY nom_user ASC, prenom_user ASC LIMIT :start, :limit";
